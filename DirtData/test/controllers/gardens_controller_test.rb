...
class GardensControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create garden" do
    assert_difference('Garden.count') do
      post gardens_url, params: { garden: { name: @garden.name, password: @garden.password } }
    end
  ...
  test "should update garden" do
    patch garden_url(@garden), params: { garden: { name: @garden.name, password: @garden.password } }
